...
test('assertCanCreateRequest blocks another advance request while unfinished advance exists', () => {
  ...
  assert.throws(
    () => ExpenditureRequestPolicy.assertCanCreateRequest(records, '通常請求'),
    /新しい通常請求はできません/,
  );
});
...
test('assertCanCreateRequest blocks another reimbursement only while a reimbursement is pending', () => {
  ...
  assert.throws(
    () => ExpenditureRequestPolicy.assertCanCreateRequest(pendingRecords, '事後請求'),
    /新しい事後請求はできません/,
  );
  assert.doesNotThrow(() =>
    ExpenditureRequestPolicy.assertCanCreateRequest(closedRecords, '事後請求'),
  );
});
...
test('validateAttachment enforces PDF uploads for reimbursement and settlement flows', () => {
  assert.throws(
    () =>
      ExpenditureRequestPolicy.validateAttachment('事後請求', {
        fileName: null,
        mimeType: null,
      }),
    /PDFファイルを添付してください/,
  ...
  assert.throws(
    () =>
      ExpenditureRequestPolicy.validateAttachment('通常精算', {
  ...
  assert.doesNotThrow(() =>
    ExpenditureRequestPolicy.validateAttachment('通常請求', {
      fileName: null,
      mimeType: null,
    }),
  );
