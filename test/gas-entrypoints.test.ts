...
test('build output exposes GAS entrypoints globally', async () => {
  ...
  assert.match(builtCode, /global\.doGet = doGet;/);
  assert.match(builtCode, /global\.getAdminDashboardBootstrap = getAdminDashboardBootstrap;/);
  assert.match(builtCode, /global\.getAdminDashboardData = getAdminDashboardData;/);
  assert.match(builtCode, /global\.getInquiryDashboardData = getInquiryDashboardData;/);
  assert.match(builtCode, /global\.updateStatus = updateStatus;/);
  assert.match(builtCode, /global\.processForm = processForm;/);
  assert.match(builtCode, /global\.processInquiry = processInquiry;/);
  assert.match(builtCode, /global\.include = include;/);
  assert.match(builtCode, /global\.authorize = authorize;/);
